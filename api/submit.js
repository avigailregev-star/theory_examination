...
const { getServiceClient } = require('./_supabase');
const { findTenant, scheduleSlot } = require('./_tenant');
...
module.exports = async function handler(req, res) {
  ...
  const { data: tenant, error: tenantError } = await findTenant(supabase, tenantSlug, 'id,slug,schedule');
  if (tenantError || !tenant) {
    res.status(404).json({ error: 'TENANT_NOT_FOUND' });
    return;
  }
...
const findTenant = (supabase, slug) => {
  if (typeof slug !== 'string' || !TENANT_SLUG.test(slug)) return Promise.resolve({ data: null, error: { message: 'TENANT_NOT_FOUND' } });
  return supabase.from('tenants').select('id,slug,schedule').eq('slug', slug).maybeSingle();
};
const scheduleSlot = (schedule, level, slotId) => {
  const slots = schedule?.[String(level)];
  return Array.isArray(slots) ? slots.find((slot) => Array.isArray(slot) && String(slot[0]) === String(slotId)) || null : null;
};
...
module.exports = async function handler(req, res) {
  ...
  if (typeof tenantSlug !== 'string' || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(tenantSlug)) {
    res.status(404).json({ error: 'TENANT_NOT_FOUND' });
    return;
  }
  const supabase = getServiceClient();
  ...
  if (!scheduleSlot(tenant.schedule, row.level, row.slot)) return res.status(400).json({ error: 'SLOT_LEVEL_MISMATCH' });

  // Do not trust a slot ID just because it appears in client-submitted JSON.
  // Resolve it again through the tenant-scoped table before booking.
  const { data: ownedSlot, error: ownedSlotError } = await supabase
    .from('lesson_slots')
    .select('id,tenant_id')
    .eq('tenant_id', tenant.id)
    .eq('id', row.slot)
    .maybeSingle();
  if (ownedSlotError) return res.status(500).json({ error: ownedSlotError.message });
  if (!ownedSlot || ownedSlot.tenant_id !== tenant.id) return res.status(400).json({ error: 'SLOT_TENANT_MISMATCH' });
